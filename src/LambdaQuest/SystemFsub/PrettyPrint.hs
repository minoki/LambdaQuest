...
prettyPrintTypeP p ctx t = case t of
  ...
  TyAll name TyTop t -> let name' = rename ctx name
                        in showParen (p > 0) $ showString "forall " . showString name' . showString ". " . prettyPrintTypeP 0 (name' : ctx) t
  TyAll name b t -> let name' = rename ctx name
                    in showParen (p > 0) $ showString "forall " . showString name' . showString "<:" . prettyPrintTypeP 1 ctx b . showString ". " . prettyPrintTypeP 0 (name' : ctx) t
...
prettyPrintTermP p tyctx ctx t = case t of
  ...
  TTyAbs name TyTop body -> let name' = rename ctx name
                            in showParen (p > 0) $ showString "?" . showString name' . showString ". " . prettyPrintTermP 0 (name' : tyctx) ctx body
  TTyAbs name b body -> let name' = rename ctx name
                        in showParen (p > 0) $ showString "?" . showString name' . showString "<:" . prettyPrintTypeP 1 tyctx b . showString ". " . prettyPrintTermP 0 (name' : tyctx) ctx body
  ...
  TCoerce x t -> showParen (p > 1) $ prettyPrintTermP 1 tyctx ctx x . showString " as " . prettyPrintTypeP 1 tyctx t
