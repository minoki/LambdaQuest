...
spec = describe "SystemF.Parse" $ do
  ...
  it "can parse a universal type" $ do
    parseType "" "forall a. a -> a -> a" `shouldBe` Right (TyAll "a" (TyArr (TyRef 0 "a") (TyArr (TyRef 1 "a") (TyRef 2 "a"))))
  ...
  it "can parse a polymorhic function" $ do
    parseTerm "" "?a. \\x: a. \\y: a . x " `shouldBe` Right (TTyAbs "a" $ TAbs "x" (TyRef 0 "a") $ TAbs "y" (TyRef 1 "a") $ TRef 1 "x")
