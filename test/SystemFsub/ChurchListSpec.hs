module SystemFsub.ChurchListSpec where
import Test.Hspec
import Data.Either
...
spec = do
  ...
  let cList t = TyAll "a" TyTop $ TyArr (TyArr t (TyArr (TyRef 1 "a") (TyRef 2 "a"))) (TyArr (TyRef 1 "a") (TyRef 2 "a"))
  ...
  describe "SystemFsub.Parse with Church lists" $ do
  ...
  let rest = do
        ...
        return $ do
          describe "SystemFsub.TypeCheck with Church lists" $ do
            it "can type check cons" $ do
              typeOf [] cons `shouldBe` Right (TyAll "t" TyTop $ TyArr (TyRef 0 "t") $ TyArr (cList (TyRef 2 "t")) (cList (TyRef 3 "t")))
            it "can type check nil" $ do
              typeOf [] nil `shouldBe` Right (TyAll "t" TyTop $ cList (TyRef 1 "t"))
            it "can type check isnil" $ do
              typeOf [] isnil `shouldBe` Right (TyAll "t" TyTop $ TyArr (cList (TyRef 1 "t")) TyBool)
          ...
          describe "SystemFsub.Eval with Church lists" $ do
