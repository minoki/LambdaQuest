module SystemFsub.ChurchNumeralSpec where
import Test.Hspec
import Data.Either
...
spec = do
  ...
  describe "SystemFsub.Parse with Church numerals" $ do
    it "can parse Nat type" $ do
      eNatType `shouldBe` Right (TyAll "a" TyTop $ TyArr (TyArr (TyRef 0 "a") (TyRef 1 "a")) (TyArr (TyRef 1 "a") (TyRef 2 "a")))
    it "can parse c0" $ do
      ec0 `shouldBe` Right (TTyAbs "a" TyTop $ TAbs "s" (TyArr (TyRef 0 "a") (TyRef 1 "a")) $ TAbs "z" (TyRef 1 "a") $ TRef 0 "z")
  ...
  let rest = do
        ...
        cnat2int <- ecnat2int
        return $ do
          describe "SystemFsub.TypeCheck with Church numerals" $ do
          ...
          describe "SystemFsub.Eval with Church numerals" $ do
            it "cnat2int c0 = 0" $ do
